...
# Variables
API_URL="https://aviationweather.gov/api/deta/metar"
OUTPUT_DIR="raw_metars"
AIRPORT_CODES_FILE="airport_codes.txt"

# Make directory using OUTPUT_DIR variable, using the -p flag so no error is thrown if directory already exists
mkdir -p "$OUTPUT_DIR"
...
while read -r airport_code; do
    ...
    curl -s --ssl-no-revoke "$URL" -o "$OUTPUT_FILE" 2>&1
